...
local SECTIONS = {
  ...
  { header = '🔧 カスタムキーマップ', color = 'ShortcutsBuffer', rows = {
    ...
    { 'Space e',         'explorerを開閉（詳細は下のセクション）' },
    { 'Space g',         'gitパネルを開閉（詳細は下のセクション）' },
    { 'Space d',         'dockerパネルを開閉（詳細は下のセクション）' },
    { 'Space P',         'ポートパネルを開閉（詳細は下のセクション）' },
    ...
    { 'Space c h',       'Call Hierarchy: 呼び出し階層を開く' },
    { 'Space f',         'LSP: フォーマット' },
    { '[d / ]d',         'LSP: 前 / 次の診断へ' },
    ...
    { 'Space /',         'search: プロジェクト内容検索を開く（詳細は下のセクション。ファイル名検索は explorer の /）' },
    { 'Space *',         'search: カーソル単語で内容検索を開く（詳細は下のセクション）' },
    { 'Space o',         'HTML / MarkdownをローカルHTTPサーバで開く' },
    { 'Space m',         'メモ帳（notes/md/ を本文検索。空で一覧、Enterで開く、Ctrl-nで新規、Ctrl-oでブラウザ一覧。内容はローカル永続）' },
  ...
  { header = '🔎 search（Space / または Space * で開く）', color = 'ShortcutsSearch', rows = {
    ...
    { 'Ctrl-x',          'マッチした全ファイルを丸ごと置換（確認なし）。置換欄が空なら何も起きない' },
    { 'Ctrl-r',          '置換欄の表示トグル。隠すと置換キーは効かなくなる' },
    { 'Ctrl-g',          'include/exclude 欄をまとめて表示トグル。隠すと絞り込みも効かなくなる' },
    { 'Alt-c',           '大文字小文字を区別する（VSCode の Aa）。既定 OFF ＝ 区別しない' },
  ...
  { header = '🔍 Peek 窓（g d / g r / g y / g I で開く）', color = 'ShortcutsBuffer', rows = {
    { 'j / k',           'カーソルを下 / 上へ（右のプレビューが追随する）' },
    { '↓ / ↑ / gg / G',  '同上。カーソル行がそのまま選択なので、マウスクリックや / 検索でも追随する' },
    { 'Tab / Shift-Tab', '次 / 前の結果へ' },
    { 'Enter',           'カーソル行の結果へジャンプ（Peek 窓は閉じる）' },
    { 'q / Esc',         '閉じる' },
  }},
  { header = '🌳 呼び出し階層（Space c h で開く）', color = 'ShortcutsBuffer', rows = {
    { 'j / k',           'カーソルを下 / 上へ（右のプレビューが追随する）' },
    { '↓ / ↑ / gg / G',  '同上。カーソル行がそのまま選択なので、マウスクリックでも追随する' },
    { 'l / Tab',         'その行をもう一段たどる（開いた枝は残るので、どこまで見たかが分かる）' },
    { 'h',               '畳む（既に閉じているなら親へ戻る）' },
    { 'i / o',           '呼び元(incoming) / 呼び先(outgoing) を切り替える' },
    { 'Enter',           'カーソル行の場所へジャンプ（呼び元なら「呼んでいるその行」へ）' },
    { 'q / Esc',         '閉じる' },
    { 'マーカーの意味',  '▸ 未展開 / ▾ 展開済み / · これ以上たどれない / ↻ 循環（祖先と同じ関数）/ … 取得中' },
  }},
  ...
  { header = '🔌 ポートパネル（Space P で開閉、1-2/←→/タブクリックでパネル切替）', color = 'ShortcutsBuffer', rows = {
    ...
    { '+',               '右ペインを拡大 / 再度で元に戻す（@と排他）' },
    { 'q / Esc',         '閉じる' },
    { 'd',               '終了（nvim の自前サーバならサーバだけ停止、それ以外は SIGTERM。確認あり）' },
    { 'D',               'プロセスを強制終了（SIGKILL、確認あり。nvim ごと落ちる）' },
