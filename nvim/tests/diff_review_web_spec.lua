...
T.describe('diff_review/web.lua', function()
  ...
  T.it('ships the unified/side-by-side toggle defaulting to unified (no persistence)', function()
    local html = web.render({})
    T.contains(html, 'id="modebtn"')
    T.contains(html, "mode: 'unified'")                  -- 既定は unified
    T.contains(html, 'renderHunkSplit')
    T.contains(html, 'renderHunkUnified')
    -- 表示系の選択は永続化しない(開くたびに既定で始まる)
    T.ok(not html:find('localStorage', 1, true), 'must not persist UI state in localStorage')
  end)
  ...
  T.it('ships uncommitted/committed review tabs in the header and an unstaged/staged lens above the tree', function()
    ...
    T.contains(html, "view: 'uncommitted'")             -- 既定は Uncommitted(永続化しない)
  ...
  T.it('can hide all comment UI to focus on the diff alone', function()
    ...
    T.contains(html, 'commentsHidden: false')           -- 既定は表示(永続化しない)
